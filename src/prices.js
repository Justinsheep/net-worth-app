...
// 美股報價、基金淨值都改由 GitHub Actions 在伺服器端抓好寫進 prices.json——
// 瀏覽器直連 Stooq / MoneyDJ 會被 CORS 擋掉（實測確認），這是唯一可靠的做法。
// 代價：剛新增一檔全新的股票/基金時當下還沒有價格，要等下一次排程（最多 30 分鐘）。
export async function fetchOneUsStockPrice() {
  return null
}
...
export async function loadPrices(holdings) {
  ...
  // ---- 台股 / 美股 / 基金：讀 prices.json（由 GitHub Actions 排程產生）----
  try {
