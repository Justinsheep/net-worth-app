...
// ---------- 成本 / 損益（統一用台幣比較）----------
// 所有有市價的分類（台股/美股/加密貨幣/基金）的 totalCost 欄位存的都是「持倉均價」
// （每顆成本），不是總投入金額——使用者自己維護的均價通常已經把手續費、稅算進去，
// 比「總投入 ÷ 數量」自動算出來的準，而且加碼/賣出都不會自動連動跑掉。
...
// 「今日漲跌幅」：只計算有市場報價、會自然漲跌的資產（台股/美股/加密貨幣現貨/基金）今天賺賠多少，
// 除以整個淨資產——代表這筆漲跌對你整體身家的影響比例。
// 存錢、轉帳、新增現金/銀行/負債都不會被算進來，因為那些不是「今天資產本身漲跌」。
const PRICED_CATEGORIES = ['tw_stock', 'us_stock', 'crypto', 'fund']
export function todayChangeRatio(holdings, fx, prices, fxRates, changePct) {
  let changeTwd = 0
  let hasAny = false
  for (const h of holdings) {
    if (!PRICED_CATEGORIES.includes(h.category) || holdingIsCashLike(h)) continue
    const pct = changePct ? changePct[priceKey(h)] : null
    if (pct == null || !Number.isFinite(pct)) continue
    const valueTwd = holdingValueTwd(h, fx, prices, fxRates)
    // pct = (今天-昨天)/昨天，所以今天的變動金額 = 現值 × pct / (1+pct)
    const denom = 1 + pct
    if (!denom) continue
    changeTwd += (valueTwd * pct) / denom
    hasAny = true
  }
  if (!hasAny) return null
  const { netWorth } = summarize(holdings, fx, prices, fxRates)
  if (!netWorth) return null
  return { changeTwd, ratio: changeTwd / netWorth }
}
